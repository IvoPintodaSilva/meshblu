...
function setupCoapRoutes(coapRouter, skynet){
  ...
  /*  register device  */
  coapRouter.post('/rd', function (req, res) {
    skynet.sendActivity(getActivity('devices', req));

    /*  read lwm2m objects file to store info  */
    var fs = require('fs');
    ...
    /*  check which key in the request params is the object uri  */
    for(var j = 0; j < Object.keys(req.params).length; j++){
      /*  regex to check if the received key matches a lwm2m uri  */
      /*      -> /^<\/([0-9])+\/([0-9])+>$/.test(Object.keys(req.params)[j])  */

      if(/^<\/([0-9])+\/([0-9])+>$/.test(Object.keys(req.params)[j])){
        /*  remove "<" and ">"  */
        var device_uri = Object.keys(req.params)[j].slice(1, -1);
        console.log(device_uri);

        /*  get lwm2m device_id and resource_id  */
        var device_lwm2m_info = device_uri.split("/");
        /*  "/3/2" .split("/") = ["", "3", "2"]  */
        object_id = device_lwm2m_info[1];
        instance_id = device_lwm2m_info[2];
    ...
    var json = JSON.parse(require('fs').readFileSync(path.dirname(require.main.filename) + '/lib/citibrain-objects-spec.json', 'utf8'));
    for(var i = 0; i < json.length; i++){
      if(json[i].id == object_id){
        console.log(json[i].name);

        var json_to_store;

        /*  -- discover the device's resources to build the json structures --  */
        /*  only working for water leakage for now  */
        object_id_found = true;

        /*  create a json for water leakage signals  */
        var json_to_store = {}

        var resources = json[i].resourcedefs;
        var payload = {}

        /*  for all the resources in the object  */
        for (var j = 0; j < json[i].resourcedefs.length; j++){
          /*  if resource is not <devices>  */
          if (resources[j].name != "devices"){
            console.log(resources[j].name + " -> /" + object_id + "/" + resources[j].id);
            if(resources[j].type == "string"){
              payload[resources[j].name] = "";
            }
            else if (resources[j].type == "integer") {
              payload[resources[j].name] = 0;
            }
          }
          /*  if resource is <devices>  */
          else{
            json_to_store[resources[j].name] = "";
          }
        }
        /*  general json discovered with some imperfections  */
        json_to_store["payload"] = payload;
        console.log(json_to_store);




        json_to_store.ipAddress = req.params.ipAddress || req.rsinfo.address;
        register(json_to_store, function (error, data) {
          if(error) {
            console.log("error");
            res.statusCode = 500;
            return res.json(error.msg);
    ...
    /*  if no object_id found in spec, just save whatever comes  */
    if(!object_id_found){
      req.params.ipAddress = req.params.ipAddress || req.rsinfo.address;
      register(req.params, function (error, data) {
        if(error) {
...
/*  lwm2m server observe device resource  */
...
// }
  ...
  /*  Sending water leakage sensor signals - example  */

  // {
  //   "devices":"WATER_TARGET_UUID",
  //   "payload":{
  //     "name":"water_event",
  //     "device_type":"sensor",
  //     "water_flow": 20
